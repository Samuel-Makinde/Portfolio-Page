import {FaBars, FaTimes, FaGithub, FaLinkedin,  FaDiscord, FaTwitter} from 'react-icons/fa'
  ...
  return (
    <div className="fixed w-full h-[60px] flex justify-between items-center bg-gray-900 text-gray-300  px-4">
      ...
      <div onClick={handleToggle} className='md:hidden z-10 '>
        ...
        {toggle ? <FaTimes className='cursor-pointer transition-transform ease-in-out delay-1000 duration-1000 transform hover:scale-110' /> : <FaBars className='cursor-pointer transition-transform hover:scale-110 ease-in-out delay-1000 duration-1000 transform' />}
      </div>
      
      <div>
        <img className='w-[40px] lg:ml-[70px] xl:ml[90px]'  src={Logo} alt="initials" />
      </div>
     ...
     {/* mobile navbar view */}
      <ul className={` ${toggle ? 'block' : 'hidden'} absolute top-0 left-0 md:hidden flex flex-col justify-center items-center w-3/4 h-screen bg-gray-900`} >
          ...
          <li  className='py-6 text-[25px]'>
            <Link onClick={handleToggle} to="contacts"  smooth={true}  duration={500} >
          Contacts
        </Link>
          </li>
        </ul>
